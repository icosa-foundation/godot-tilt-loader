...
The current backend wraps QuickHull. That is fast enough for the cafe hull smoke test, but it is not bit-for-bit or topology-for-topology identical to Unity's `MIConvexHull35.dll`. Representative regular hull samples match Unity/MIConvexHull point counts and fan-triangulated triangle counts at the calibrated tolerance. The native wrapper returns ordered polygon faces for truly coplanar facets, so polygon face count can be lower than triangle count. Two of the three sampled ConcaveHull windows match; ConcaveHull stroke 95's final window is a known mismatch where MIConvexHull returns 6 points / 8 fan triangles and QuickHull returns 8 points / 12 fan triangles for the same 10 input points.
