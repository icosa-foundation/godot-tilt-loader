...
func _check_math_utils() -> void:
	...
	var reflected_unity_forward := Vector3.FORWARD
	var frame := MathUtils.compute_minimal_rotation_frame(reflected_unity_forward, null, Quaternion.IDENTITY)
	_expect_vec3_close(Basis(frame) * Vector3.FORWARD, reflected_unity_forward, "Minimal frame reflected Unity forward")
