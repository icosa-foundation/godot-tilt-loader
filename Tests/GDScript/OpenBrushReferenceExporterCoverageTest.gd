...
const REFERENCE_COMPARATOR_PATH := "res://Tests/GDScript/OpenBrushReferenceMeshFixtureTest.gd"
...
func _init() -> void:
	...
	var reference_comparator := _read_text(REFERENCE_COMPARATOR_PATH)
	...
	_expect(not reference_comparator.is_empty(), "reference comparator loads")
	...
	if not exporter_source.is_empty():
		...
		_expect(exporter_source.contains("particle_attributes"), "exporter serializes particle attribute layout flag")
		_expect(exporter_source.contains("bUseVertexIds"), "exporter serializes vertex-id layout flag")
		_expect(exporter_source.contains("bFbxExportNormalAsTexcoord1"), "exporter serializes particle normal-remap layout flag")
		_expect(exporter_source.contains("normal_semantic"), "exporter serializes normal semantic")
		_expect(exporter_source.contains("uv0_semantic"), "exporter serializes uv semantics")
	...
	if not exporter_runner.is_empty():
		...
		_expect(exporter_runner.contains("OpenBrushReferenceMeshExportTest.cs"), "runner stages exporter test source")
		_expect(exporter_runner.contains("Assets\\Editor\\Tests"), "runner stages exporter under Unity editor tests")
		_expect(exporter_runner.contains("Copy-Item"), "runner copies exporter source into target worktree")

	if not reference_comparator.is_empty():
		_expect(reference_comparator.contains("_layout_has_particle_attributes"), "comparator detects particle layout contract")
		_expect(reference_comparator.contains("_compare_particle_render_arrays"), "comparator checks particle render arrays")
		_expect(reference_comparator.contains("Mesh.ARRAY_CUSTOM0"), "comparator checks particle custom data")
		_expect(reference_comparator.contains("Mesh.ARRAY_TEX_UV2"), "comparator checks particle UV2 data")
	...
	if not reference_readme.is_empty():
		...
		_expect(reference_readme.contains("copies the exporter source"), "README documents automatic exporter staging")
		_expect(reference_readme.contains("particle_attributes"), "README documents particle attribute layout flag")
		_expect(reference_readme.contains("UV/UV2/TANGENT/CUSTOM0"), "README documents particle render remap")
