...
func _init() -> void:
	...
	if manifest != null:
		...
		_check_catalog_flat_strip_prefabs(manifest)
...
func _check_catalog_flat_strip_prefabs(manifest: TiltBrushManifest) -> void:
	var compatibility := {}
...
func _check_replay(desc: BrushDescriptor, prefab: String) -> void:
	...
	stroke.m_BrushSize = 0.18
	stroke.m_Color = Color(0.95, 0.35, 0.18, 1.0)
	stroke.m_Seed = 12345
	...
	print("FLAT_STRIP_CATALOG_REPLAY\tbrush=%s\tprefab=%s\truntime=%s\tverts=%d\ttris=%d\tuv0_v2=%d\tuv0_v3=%d\tuv1_v3=%d\ttangents=%d" % [
		desc.m_DurableName,
		prefab,
		runtime_class,
		vertex_count,
	...
	match prefab:
		"Line", "LineWithWidth":
			_expect_equal(runtime_class, "QuadStripBrushStretchUV", "%s runtime class" % desc.m_DurableName)
			_expect(vertex_count % 6 == 0, "%s quad-strip vertex count is quad aligned" % desc.m_DurableName)
			if bool(desc.prefab_fields.get("m_StoreWidthInTexcoord0Z", false)):
				_expect_equal(mesh_data.uv0_v3.size(), vertex_count, "%s width uv0 Vector3 count" % desc.m_DurableName)
				_expect_equal(mesh_data.uv0_v2.size(), 0, "%s width uv0 Vector2 count" % desc.m_DurableName)
			else:
			...
			_expect_equal(mesh_data.uv1_v3.size(), 0, "%s quad-strip uv1 Vector3 count" % desc.m_DurableName)
		"DistanceUV":
			_expect_equal(runtime_class, "QuadStripBrushDistanceUV", "%s runtime class" % desc.m_DurableName)
			_expect(vertex_count % 6 == 0, "%s quad-strip vertex count is quad aligned" % desc.m_DurableName)
			_expect_equal(mesh_data.uv0_v2.size(), vertex_count, "%s uv0 Vector2 count" % desc.m_DurableName)
			_expect_equal(mesh_data.uv0_v3.size(), 0, "%s uv0 Vector3 count" % desc.m_DurableName)
			_expect_equal(mesh_data.uv1_v3.size(), 0, "%s distance uv1 Vector3 count" % desc.m_DurableName)
		"UnitizedUV":
			_expect_equal(runtime_class, "QuadStripUnitizedUVBrush", "%s runtime class" % desc.m_DurableName)
			_expect(vertex_count % 6 == 0, "%s quad-strip vertex count is quad aligned" % desc.m_DurableName)
			_expect_equal(mesh_data.uv0_v2.size(), vertex_count, "%s uv0 Vector2 count" % desc.m_DurableName)
			_expect_equal(mesh_data.uv0_v3.size(), 0, "%s uv0 Vector3 count" % desc.m_DurableName)
			_expect_equal(mesh_data.uv1_v3.size(), 0, "%s unitized uv1 Vector3 count" % desc.m_DurableName)
		"FlatDistance", "FlatStretch", "MidpointPlusOffset":
			_expect_equal(runtime_class, "FlatGeometryBrush", "%s runtime class" % desc.m_DurableName)
			_expect_equal(mesh_data.uv0_v2.size(), vertex_count, "%s uv0 Vector2 count" % desc.m_DurableName)
			_expect_equal(mesh_data.uv0_v3.size(), 0, "%s uv0 Vector3 count" % desc.m_DurableName)
			if bool(desc.prefab_fields.get("m_bOffsetInTexcoord1", false)):
				_expect_equal(mesh_data.uv1_v3.size(), vertex_count, "%s offset uv1 Vector3 count" % desc.m_DurableName)
			else:
				_expect_equal(mesh_data.uv1_v3.size(), 0, "%s flat uv1 Vector3 count" % desc.m_DurableName)

func _sample_points() -> Array[Dictionary]:
	return [
		{
			"position": Vector3(0.0, 0.0, 0.0),
			"orientation": Quaternion(Vector3.UP, 0.0),
			"pressure": 1.0,
			"timestamp": 0,
		},
		{
			"position": Vector3(0.75, 0.05, 0.0),
			"orientation": Quaternion(Vector3.UP, 0.18),
			"pressure": 0.85,
			"timestamp": 16,
		},
		{
			"position": Vector3(1.35, 0.2, 0.18),
			"orientation": Quaternion(Vector3.UP, 0.42),
			"pressure": 1.0,
			"timestamp": 33,
		},
		{
			"position": Vector3(2.0, 0.12, -0.12),
			"orientation": Quaternion(Vector3.UP, 0.68),
			"pressure": 0.92,
			"timestamp": 50,
		},
		{
			"position": Vector3(2.7, -0.05, 0.16),
			"orientation": Quaternion(Vector3.UP, 0.94),
			"pressure": 0.78,
			"timestamp": 67,
		},
		{
			"position": Vector3(3.35, 0.08, 0.04),
			"orientation": Quaternion(Vector3.UP, 1.15),
			"pressure": 0.96,
			"timestamp": 84,
		},
	]
...
func _expect(condition: bool, message: String) -> void:
	if not condition:
		_failures += 1
		push_error("FlatStripCatalogReplayTest: %s" % message)

func _expect_equal(actual: Variant, expected: Variant, message: String) -> void:
	if actual != expected:
		_failures += 1
		push_error("FlatStripCatalogReplayTest: %s expected %s but got %s" % [message, expected, actual])
