@tool
class_name OpenBrushTiltReader
...
func _read_zip_data(path: String) -> Dictionary:
	...
	if err != OK:
		...
		if err != OK:
			push_warning("OpenBrushTiltReader: ZIPReader failed to open %s (error %d)" % [path, err])
			return result
...
func _parse_sketch(data: PackedByteArray, brush_index: Array) -> Array:
	...
	if sentinel != SKETCH_SENTINEL:
		push_warning("OpenBrushTiltReader: invalid sketch sentinel 0x%x" % sentinel)
		return []
	...
	for i in range(num_strokes):
		...
		if stroke.is_empty():
			...
			push_warning("OpenBrushTiltReader: stroke %d parse failed, stopping." % i)
			break
		strokes[i] = stroke
