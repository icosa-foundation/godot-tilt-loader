...
namespace TiltBrush
{
	...
	// - Parallel transport?
	public partial class ConcaveHullBrush : GeometryBrush
	{
		...
		void OnChanged_MakeGeometry(int iKnot0)
		{
			...
			for (int iKnot = iKnot0; iKnot < m_knots.Count; ++iKnot)
			{
...

				{
					...
					ConvexHullCreationResult<Vertex, Face> hull = CreateHull(input, enableSimplify: false);
					if (hull != null)
					{
						if (m_Faceted)
						{
		...
		void CreateSmoothGeometry(ref Knot knot, ConvexHullCreationResult<Vertex, Face> hull)
		{
			...
			Vertex[] hullPoints = hull.Result.Points.ToArray();
