class_name MidpointPlusLifetimeSprayBrush
...
const K_SALT_MAX_QUADS_PER_KNOT := 5
...
func get_vertex_layout(_desc: BrushDescriptor) -> GeometryPool.VertexLayout:
	return GeometryPool.VertexLayout.create(
		...
		GeometryPool.TexcoordInfo.create(4, GeometryPool.Semantic.VECTOR),
		null,
		true,
		true,
		true
	...
	for index in range(knot_index, m_knots.size()):
		...
		if cur.has_geometry():
			...
			for quad in range(num_quads):
				var salt := K_SALT_MAX_SALTS_PER_QUAD * (index * K_SALT_MAX_QUADS_PER_KNOT + quad)
				...
				set_uv1(vert_index, BL, _offset4(-forward_offset - right_offset, creation_time))
				set_vert(vert_index, FR, center + forward_offset + right_offset, cur.nSurface, m_Color, alpha)
				set_uv1(vert_index, FR, _offset4(forward_offset + right_offset, creation_time))
				set_vert(vert_index, FL, center + forward_offset - right_offset, cur.nSurface, m_Color, alpha)
				set_uv1(vert_index, FL, _offset4(forward_offset - right_offset, creation_time))
