class_name TiltAccurateViewer
extends Node3D

@export var TiltFilePath := "C:/Users/andyb/Documents/Open Brush/Sketches/brush_cafe_experimental.tilt"
...
func _ready() -> void:
	...
	_reset_log()
	_log("TILT_ACCURATE: loading %s" % TiltFilePath)

	var reader_script := load(TILT_READER_PATH)
	if reader_script == null:
		_fail("TILT_ACCURATE: missing reader at %s" % TILT_READER_PATH)
		return
	...
	if not reader_error.is_empty():
		_fail("TILT_ACCURATE: reader error: %s" % reader_error)
		return
	...
	if imported is PackedScene:
		...
		_fail("TILT_ACCURATE: %s did not load as an imported PackedScene; run Godot import first" % TiltFilePath)
		return
	if _scene_root == null:
		_fail("TILT_ACCURATE: imported scene instantiated as null")
		return
	...
	if stats.mesh_instances == 0 or stats.vertices == 0:
		_fail("TILT_ACCURATE: imported scene had no renderable mesh")
		return

	_add_camera(stats.bounds)
	_add_world()
	_log("TILT_ACCURATE: imported strokes=%d mesh_instances=%d vertices=%d triangles=%d materials=%s bounds_min=%s bounds_max=%s" % [
...
func _apply_brush_filter(root: Node) -> void:
	...
	_log("TILT_ACCURATE: brush filter=%s" % ", ".join(OnlyBrushes))
...
func _force_double_sided(root: Node) -> void:
	...
	_log("TILT_ACCURATE: forced StandardMaterial3D cull disabled")
...
func _save_thumbnail(bytes: PackedByteArray) -> void:
	if bytes.is_empty():
		_log("TILT_ACCURATE: no embedded thumbnail found")
		return
	var file := FileAccess.open(ThumbnailOutputPath, FileAccess.WRITE)
	if file == null:
		_log("TILT_ACCURATE: failed to save thumbnail to %s error=%d" % [ThumbnailOutputPath, FileAccess.get_open_error()])
		return
	file.store_buffer(bytes)
	file.close()
	_log("TILT_ACCURATE: saved embedded thumbnail to %s bytes=%d" % [ProjectSettings.globalize_path(ThumbnailOutputPath), bytes.size()])
...
func _save_render() -> void:
	...
	if texture == null:
		_fail("TILT_ACCURATE: viewport texture is null; run with a real display driver for screenshot capture")
		return
	var image := texture.get_image()
	if image == null:
		_fail("TILT_ACCURATE: viewport image is null; run with a real display driver for screenshot capture")
		return
	var err := image.save_png(RenderOutputPath)
	if err != OK:
		_fail("TILT_ACCURATE: failed to save render to %s error=%d" % [RenderOutputPath, err])
		return
	var pixel_stats := _count_visible_pixels(image)
	_log("TILT_ACCURATE: saved render to %s size=%dx%d non_background_pixels=%d" % [
