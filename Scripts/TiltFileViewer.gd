...
func _ready() -> void:
	...
	_reset_log()
	_log("TILT_VIEWER: loading %s" % TiltFilePath)

	var reader_script := load(TILT_READER_PATH)
	if reader_script == null:
		_fail("TILT_VIEWER: missing tilt reader at %s" % TILT_READER_PATH)
		return
	...
	if not reader_error.is_empty():
		_fail("TILT_VIEWER: reader error: %s" % reader_error)
		return

	var imported := load(TiltFilePath)
	if not imported is PackedScene:
		_fail("TILT_VIEWER: %s did not load as an imported PackedScene; reimport the .tilt file" % TiltFilePath)
		return

	_scene_root = imported.instantiate()
	if _scene_root == null:
		_fail("TILT_VIEWER: imported scene instantiated as null")
		return
	...
	if stats.mesh_instances == 0 or stats.vertices == 0:
		_fail("TILT_VIEWER: imported scene had no renderable mesh")
		return

	_add_camera(stats.bounds)
	_add_world()
	_log("TILT_VIEWER: imported strokes=%d mesh_instances=%d vertices=%d triangles=%d bounds_min=%s bounds_max=%s" % [
	...
	if _quit_after_screenshot:
		call_deferred("_save_render_after_frames")
...
func _apply_command_line_args() -> void:
	for arg in OS.get_cmdline_user_args():
		if arg == "--quit-after-screenshot":
			...
			_log_path = arg.trim_prefix("--log-output=")
...
func _add_world() -> void:
	...
	world.name = "TiltViewerWorldEnvironment"
...
func _save_render() -> void:
	...
	if texture == null:
		_fail("TILT_VIEWER: viewport texture is null; run with a real display driver for screenshot capture")
		return
	var image := texture.get_image()
	if image == null:
		_fail("TILT_VIEWER: viewport image is null; run with a real display driver for screenshot capture")
		return
	var err := image.save_png(RenderOutputPath)
	if err != OK:
		_fail("TILT_VIEWER: failed to save render to %s error=%d" % [RenderOutputPath, err])
		return
	...
	_log("TILT_VIEWER: saved render to %s size=%dx%d non_background_pixels=%d" % [
