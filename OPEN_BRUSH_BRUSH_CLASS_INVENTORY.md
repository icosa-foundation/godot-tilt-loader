...
| `FlatGeometryBrush.gd` | `FlatGeometryBrush.cs` | `FlatDistance`, `FlatStretch`, `MidpointPlusOffset`. | Flat ribbon geometry, distance/stretch UV, optional offset in texcoord1. | Batched finalization trims short post-break tails like Open Brush. | `FlatGeometryBrushParityTest.gd`, cafe DuctTapeGeometry fixture replay. | Partially audited; distance/stretch atlas branches now use the descriptor texture-atlas count directly like Open Brush; UV1 offset vectors, non-M11 smoothing/shared-vertex output, continuation tangents, and double-back break behavior now have focused coverage. |
