...
- Godot C# source directory: `Assets/Scripts/Brushes`
...
| `QuadStripBrush.gd` | `QuadStripBrush.cs` | Base for `Line`, `LineWithWidth`, `UnitizedUV`, `DistanceUV`. | Flat strip topology, bend/shrink/break logic, double-sided backsides. | Solitary and batched paths must preserve strip breaks and weld single-sided strips like Open Brush. | `QuadStripParityTest.gd`, cafe Ink fixture replay. | Partially audited; spawn interval, pressure smoothing, out-of-verts threshold, small-move no-update behavior, short preview move state restoration, strip-break disabled behavior, bend/shrink/break logic, previous lone-segment squash cleanup, backside consistency, debug used counts, `GetNumUsedVerts` edge cases, preview reset layout/index behavior, destroy-time geometry-pool release, single-sided batched welding including UV0.z, and double-sided batched non-weld export have focused coverage. |
| `QuadStripBrushStretchUV.gd` | `QuadStripBrushStretchUV.cs` | `Line`, `LineWithWidth`. | Primary diffuse stretch UV; optional width stored in UV0.z. | Must flush pending UV/tangent data before final mesh export. | `QuadStripParityTest.gd`, metadata test. | Partially audited; UV method formulas and pending-request behavior are line-by-line compared to Open Brush, with Godot runtime export flushes documented as integration-specific parity plumbing. |
| `QuadStripBrushDistanceUV.gd` | `QuadStripBrushDistanceUV.cs` | `DistanceUV`. | Primary diffuse distance UV plus opacity/fade behavior. | Must flush pending tangent requests in solitary and batched finalization. | `QuadStripParityTest.gd`, metadata test. | Partially audited; distance UV, opacity fade, tangent-request union, preview reset, and no-op per-quad hook are line-by-line compared to Open Brush. |
| `QuadStripUnitizedUVBrush.gd` | `QuadStripUnitizedUVBrush.cs` | `UnitizedUV`. | Unitized strip UV behavior. | Inherits quad-strip finalization and backside requirements. | `QuadStripParityTest.gd`, metadata test. | Partially audited; unitized UV layout and no-op per-quad/per-segment hooks are line-by-line compared to Open Brush. |
| `FlatGeometryBrush.gd` | `FlatGeometryBrush.cs` | `FlatDistance`, `FlatStretch`, `MidpointPlusOffset`. | Flat ribbon geometry, distance/stretch UV, optional offset in texcoord1. | Batched finalization trims short post-break tails like Open Brush. | `FlatGeometryBrushParityTest.gd`, cafe DuctTapeGeometry fixture replay. | Partially audited; distance/stretch atlas branches now use the descriptor texture-atlas count directly like Open Brush; UV1 offset vectors, non-M11 smoothing/shared-vertex output, continuation tangents, double-back break behavior, self-intersection size clipping, growth limiting, and M11 break-angle behavior now have focused coverage. |
...
| `TubeBrush.gd` | `TubeBrush.cs` | `TubeDistanceUV`, `TubeDistanceUVSin`, `TubeStretchUV`, `Tube_Petal`, `Tube_Rain`, `Tube_Sparks`, `Tube_Spikes`, `Tube_Tapered`, `TubeBrush_Comet`, `Lofted`, `LoftedHueShift`. | Tube/ring geometry, cap options, distance/stretch UV, shape modifiers. | Uses Open Brush minimal-frame and tube finalization behavior. | `TubeBrushParityTest.gd`, cafe Sparks fixture replay, full-corpus BubbleWand fixture replay. | Partially audited. |
| `BubbleWandBrush.gd` | `BubbleWandBrush.cs` | No active normal catalog prefab currently routes here; the current `BubbleWand` asset uses `TubeStretchUV.prefab`, which references `TubeBrush.cs`. | Tube-derived bubble post-processing, UVW formula, original-position UV1 storage. | Bubble-specific solitary finalization must match Open Brush if an asset routes to this class. | `BubbleWandBrushParityTest.gd`. | Source class covered but not active in the live catalog. |
...
| `SprayBrush.gd` | `SprayBrush.cs` | `Spray`. | Spray particle quads, salt wraparound, preview decay. | Particle finalization must preserve Open Brush lifecycle semantics. | `SprayBrushParityTest.gd`. | Partially audited; spawn interval, seeded random size/rotation/position/alpha branches, atlas UV selection, multi-particle spacing, tangent/backface output, salt wraparound, preview decay, and finalization have focused coverage. Full reference fixture comparison remains. |
| `GeniusParticlesBrush.gd` | `GeniusParticlesBrush.cs` | `GeniusParticle`. | Genius particle quads, atlas UVs, randomized alpha/offset/roll, Godot particle-shader attribute export. | Must run particle finalization, remove the hanging particle, and export generated mesh attributes in the same shape as imported particle meshes. | `GeniusParticlesBrushParityTest.gd`, `MeshDataArrayExportParityTest.gd`, cafe Stars fixture replay. | Partially audited; pointer-travel spawning, preview decay length-cache reduction, decayed-knot salt offset, layout, atlas, randomized alpha/offset/roll, Godot C# port birth-time behavior, generated ArrayMesh particle attribute remap, and finalization have focused coverage. Full reference mesh comparison remains. |
| `MidpointPlusLifetimeSprayBrush.gd` | `MidpointPlusLifetimeSprayBrush.cs` | `MiddpointPlusLifetimeGeomSpray`. | Midpoint lifetime particle geometry, UV0/UV1/tangent/color output, birth time in UV1.w. | Must preserve generated midpoint particles and avoid Genius-specific finalization behavior. | `MidpointSprayBrushParityTest.gd`. | Partially audited; spawn interval, seeded random size/rotation/position/alpha branches, atlas UV selection, multi-particle spacing, tangent output, UV1 offset packing, birth time, and finalization have focused coverage. Full reference fixture comparison remains. |
...
| `SquareBrush.gd` | `SquareBrush.cs` | `SquareBrush_prefab`. | Square strip/ring geometry, caps, shared-ring continuation. | Must handle sharp-turn segment breaks, short-segment break/restarts, invisible-back framing, and cap topology like Open Brush. | `SquareBrushParityTest.gd`. | Partially audited; layout/spawn interval, straight topology, shared-ring continuation, sharp-turn breaks, short-segment restarts, invisible-back framing, and color writes have focused coverage. |
| `Square3DPrintBrush.gd` | `Square3DPrintBrush.cs` | `Square3DPrintBrush`. | 3D-print square topology and parity-flip ring-face branch. | Must preserve layout/spawn interval clamps, flip-branch topology, and break/restart behavior. | `Square3DPrintBrushParityTest.gd`. | Partially audited; layout/spawn interval clamps, single segment topology, shared-ring continuation, flip-branch topology, close-knot restarts, and color writes have focused coverage. |
| `SliceBrush.gd` | `SliceBrush.cs` | `Slice`. | Slice shared-quad geometry and UVW distance accumulation. | Uses direct geometry output with Open Brush frame direction. | `SliceBrushParityTest.gd`. | Partially audited; spawn interval/layout, shared-quad continuation, short-segment break/restart behavior, UVW distance reset, penultimate detection, winding, color, and initial normal direction have focused coverage. Full reference fixture comparison remains. |
| `PrintableBrush.gd` | `PrintableBrush.cs` | No active normal catalog prefab currently routes here. | Printable strip/ring geometry, envelope behavior, caps. | Must handle shared-ring continuation, sharp-turn breaks, short-segment break/restarts, invisible-back framing, and envelope topology like Open Brush. | `PrintableBrushParityTest.gd`. | Partially audited; layout/spawn interval, straight topology, envelope behavior, shared-ring continuation, sharp-turn breaks, short-segment restarts, invisible-back framing, and color writes have focused coverage; source class covered but not active live catalog route. |
...
| `TubeStretchUV` | `TubeBrush` | 2 | `BubbleWand`, `MylarTube` |
