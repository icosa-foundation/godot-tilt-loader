...
| `Tests/GDScript/LiveVsTiltUvParityTest.gd` | `path equivalence` | Direct replay, pointer-memory replay, pointer math, and live object path produce matching primary UVs for selected brushes. | Compares Godot paths to each other, not Open Brush reference meshes. |
