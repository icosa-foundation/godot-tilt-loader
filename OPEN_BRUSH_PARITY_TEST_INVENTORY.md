...
| `Tests/GDScript/GeniusParticlesBrushParityTest.gd` | `generated mesh parity` | Genius particle geometry, UV channels, atlas branch, pointer-travel distance override, straight-edge proxy flag, random alpha/offset/roll formulas, birth-time sign, preview decay length-cache reduction, decayed-knot salt offset, and hanging-particle finalization. | Still not compared against Open Brush reference mesh fixtures. |
...
| `Tests/GDScript/TiltImporterRuntimeReplayTest.gd` | `importer path test` | Importer has no old fallback tessellator entry points, no importer-local brush-family factory tokens, and cafe `.tilt` rebuilds through registry/replay/material resolver routing with substantial runtime geometry. | Heavy scene-level check, not per-brush numeric reference parity. |
