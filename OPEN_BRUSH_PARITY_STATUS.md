...
- `Tests/GDScript/OpenBrushReferenceMeshFixtureTest.gd` reads raw `.mesh.json` files directly from an explicitly supplied Open Brush `Support/BrushFixtures` directory. Ordinary brushes, including `ConcaveHull`, retain strict topology and vertex-channel comparison. The five regular convex-hull brushes currently use a geometric-equivalence gate while the exact polygon-face matcher is retained for later work. The current corpus result is 94 passes and one strict `ConcaveHull` failure.
